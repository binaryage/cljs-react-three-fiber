  ...
  (let [{:keys [children v-align h-align size color]
        ...
        font-config-fn (fn []
                         #js {:font           font
                              ...
                              :bevelOffset    0
                              :bevelSegments  8})
        font-config (uix/memo font-config-fn [font])
        update-fn (fn [self]
    ...
    [:group {:ref group-ref}
     [:scene (merge {:name "Scene"} props)
      [:mesh {:name                  "Object_0"
       ...
       [:meshStandardMaterial (merge {:attach "material"} (bean (get-gltf-material gltf 1)) {:name "Material_0_COLOR_0"})]]]]))
